...
int _stdcall Event_Group(int type, text parameter) {
	...
	try//抛异常以免出错导致框架崩溃
	{
		...
		case 事件_群_文本消息: {
			...
			if ( !strcmp ( "3222973652" , qq.c_str ( ) ) ) {
				if ( !strcmp ( "来个红包" , msg.c_str ( ) ) )
				{
				nt.OutPut(nt.sendExclusivePacket(stoi(group.c_str()),1,"0.01" ,"你的红包来了"));
				}

				if (!strcmp("test", msg.c_str()))
				{
					list<GroupMemberListInfo> info;
					info = nt.GetGroutMemberListInformation("1063565107");
					
					nt.OutPut(info.front().nickName.c_str());
				}
